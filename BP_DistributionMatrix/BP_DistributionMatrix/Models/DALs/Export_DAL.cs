...
public class Export_DAL
{
    ...
    public List<Companies_Model> GetAllCompanies() {
    ...
    {
        string query = @"SELECT id, contractorCode
                        FROM ddm.Companies
                        WHERE id = @CompanyID;";
        ...
        using (SqlConnection connection = new SqlConnection(_connectionString))
        {
            using (SqlCommand cmd = new SqlCommand(query, connection))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        res = reader["contractorCode"].ToString();
                        if (string.IsNullOrEmpty(res)) res = "NOCODE";
                    }
                }
    ...
    public List<Export_Model> GetAllRowsV2(int userid, string company_id)
    {
        string query = @"SELECT  
...
GROUP BY  
    ...
    ConsolidatorData.consolidators;";
        ...
        List<Export_Model> res = new List<Export_Model>();
            ...
            using (SqlCommand cmd = new SqlCommand(query, connection))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        Export_Model model = new Export_Model
                        {
                            User_Id = reader["user_id"].ToString(),
                            Doc_type_disc_id = reader["doc_type_disc_id"].ToString(),
                            Discipline = reader["disc_code"].ToString(),
                            Doc_Type = reader["doc_code"].ToString(),
                            Contractor = reader["company_id"].ToString(),
                            UserAction = reader["value"].ToString(),
